...
$(function () {
    ...
    $("#searchFlight").submit(function (event) {
        ...
        var error = false; var _data = {};
        ($(this).serializeArray()).map(e => { _data[e.name] = e.value }); console.log(_data);
        _data.fun = 'searchFlight';

        $.ajax({
            data: _data, type: "POST", url: form_action, dataType: "json", encode: true,
        }).done(function (data) {
...
function groupNavigationItems(data) {
    ...
    data.forEach(item => {
        ...
        if (group == '-') {
            ...
            li.className = 'mb-4'
            li.innerHTML = `<a class="d-flex align-items-center" href="/${item._id}" ><i class="fa-regular fa-flag pe-2"></i>${item.path}</a>`
...
function renderGroupedNavigationMenu(data) {
    ...
    for (const group in groupedItems) {
        ...
        li.className = 'mb-4'
        var str = `
        <a class="btn-toggle d-flex align-items-center collapsed" data-bs-toggle="collapse" data-bs-target="#a${++nGroup}" aria-expanded="false">
            <i class="fa-solid fa-chart-line pe-2"></i>${group}
        </a>
        <div class="collapse" id="a${nGroup}">
            <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small">`

        groupItems.forEach(item => {
            str += `<li><a href="/${item._id}" class="link-body-emphasis d-inline-flex text-decoration-none align-items-center"><i class="fa-solid fa-caret-right me-2"></i>${item.path}</a></li>`
        })
        str += `</ul></div>`
        li.innerHTML = str;
        mainNav.appendChild(li)
