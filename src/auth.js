...
router.get('/', async function (req, res, next) {
  ...
  var _agents = await mongodbClient.db(process.env.MONGO_DB_DB).collection('agency').find({}).toArray();
  var _agentsOption = '';
  _agents.forEach((x, i) => {
          ...
          <div class="col-xxl-4 col-lg-5">
              <div class="card">
                  ...
                  <div class="card-header py-3 text-center bg-primary ">
                      <a href="#">
                          <span><img src="https://manntravel.com.au/wp-content/uploads/2022/06/logo.png" class="rounded" alt="logo" height="45"></span>
                      </a>
                  </div>
...
router.post('/', async function (req, res, next) {
  ...
  if (validator.isEmail(email) && /*ObjectId.isValid(agent), */ password.length > 2 /* && password.length < 10 */ ) {
    const _user = await mongodbClient.db(process.env.MONGO_DB_DB).collection('users').findOne({ agency: new ObjectId(agency), email: email, password: password });
    console.log('_user', _user)
    if (_user != null) {

      req.session.auth = _user;

      const _agency = await mongodbClient.db(process.env.MONGO_DB_DB).collection('agency').findOne({ _id: new ObjectId(agency), status: `Active` });
      console.log('_agency', { _id: new ObjectId(agency), status: `Active` }, _agency)
      req.session.agency = _agency;

      const _interface = await mongodbClient.db(process.env.MONGO_DB_DB).collection('interface').find({ type: 'nav', agency: _user.agency }, { projection: { agency: 0 }, }).sort({ disp_seq: 1 }).toArray();
