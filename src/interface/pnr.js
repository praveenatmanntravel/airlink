...
module.exports = {
    ...
    dataForPnrView: async (req, res, next) => {
        ...
        const pnrdetails = await mongodbClient.db(process.env.MONGO_DB_DB).collection('pnrs').findOne({ _id: new ObjectId(pnrid) })
    ...
    PnrActivityLog: async (req, res, next) => {
        const pnrid = req.body?.pnrid
        const activityLogs = await mongodbClient.db(process.env.MONGO_DB_DB).collection('activity_log').find({ pnr_id: new ObjectId(pnrid) }).toArray()
